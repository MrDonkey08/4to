...
const espesorCuerpoComercial = document.getElementById(
  "espesor-cuerpo-comercial"
)
const espesorTapasCalculado = document.getElementById("espesor-tapas-calculado")
const espesorCuerpoCalculado = document.getElementById(
  "espesor-cuerpo-calculado"
)
...
function calcularRecipiente() {
  ...
  tCalTapas =
    (presion.value * diametro.value) / (2 * eta * S - 0.2 * presion.value)
  tCalCuerpo =
    (presion.value * (diametro.value / 2)) / (eta * S - 0.6 * presion.value)
  ...
  vTapas =
    (Math.PI / 12) *
    (Math.pow(diametro1 + 2 * tComercialTapa * 0.0254, 3) -
      Math.pow(diametro1, 3))
  vCuerpo =
    ((Math.PI * longitud1) / 4) *
    (Math.pow(diametro1 + 2 * tComercialBody * 0.0254, 2) -
      Math.pow(diametro1, 2))
